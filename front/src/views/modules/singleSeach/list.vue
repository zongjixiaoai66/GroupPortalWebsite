        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '名字' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.singleSeachName" placeholder="名字" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('singleSeach','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jituanmenhu/upload/singleSeachMuBan.xls"
                        >批量导入单页数据数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('singleSeach','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="singleSeachUploadSuccess"
                                :on-error="singleSeachUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('singleSeach','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入单页数据数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('singleSeach','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "singleSeach.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="singleSeachName"
                                   header-align="center"
                                   label="名字">
                        <template slot-scope="scope">
                            {{scope.row.singleSeachName}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="singleSeachTypes"
                                      header-align="center"
                                      label="数据类型">
                        <template slot-scope="scope">
                            {{scope.row.singleSeachValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="singleSeachPhoto"
                                     header-align="center"
                                     width="200"
                                     label="图片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.singleSeachPhoto">
                                <img :src="$base.url+scope.row.singleSeachPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('singleSeach','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('singleSeach','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('singleSeach','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                singleSeachName : null,
                singleSeachTypes : null,
                singleSeachPhoto : null,
                singleSeachContent : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'single_seach',//当前表表名,
                        sumColum : 'single_seach_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'single_seach_name',//分组字符串字段
                        // types : 'single_seach_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "single_seach_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.singleSeachName!= '' && this.searchForm.singleSeachName!= undefined) {
                    params['singleSeachName'] = '%' + this.searchForm.singleSeachName + '%'
                }
                                
                params['singleSeachDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "singleSeach/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            singleSeachUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "singleSeach/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
