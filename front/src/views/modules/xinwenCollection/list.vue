        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '新闻标题' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.xinwenName" placeholder="新闻标题" clearable></el-input>
                    </el-form-item>
                     
                    <el-form-item :label="contents.inputTitle == 1 ? '新闻类型' : ''">
                        <el-select v-model="searchForm.xinwenTypes" placeholder="请选择新闻类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in xinwenTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('xinwenCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xinwenCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('xinwenCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('xinwenCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jituanmenhu/upload/xinwenCollectionMuBan.xls"
                        >批量导入集团新闻收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('xinwenCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="xinwenCollectionUploadSuccess"
                                :on-error="xinwenCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('xinwenCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入集团新闻收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('xinwenCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "xinwenCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('xinwenCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="新闻类型">
                        <template slot-scope="scope">
                            {{scope.row.xinwenValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="xinwenPhoto"
                               header-align="center"
                               width="200"
                               label="新闻图片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.xinwenPhoto">
                                <img :src="$base.url+scope.row.xinwenPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xinwenCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.xinwenCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('xinwenCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('xinwenCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('xinwenCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    export default {
        ...
        return {
            ...
            userId:"",//当前登录人的id
    //级联表下拉框搜索条件
              xinwenTypesSelectSearch : [],
    //当前表下拉框搜索条件
            form:{
                id : null,
                xinwenId : null,
                yonghuId : null,
                xinwenCollectionTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'xinwen_collection',//当前表表名,
                        sumColum : 'xinwen_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'xinwen_collection_name',//分组字符串字段
                        // types : 'xinwen_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "xinwen_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                let params = {
                    ...
                    limit: this.pageSize,
                    sort: 'id',
                    xinwenCollectionTypes: 1,
                }

                                         
                if (this.searchForm.xinwenName!= '' && this.searchForm.xinwenName!= undefined) {
                    params['xinwenName'] = '%' + this.searchForm.xinwenName + '%'
                }
                     
                if (this.searchForm.xinwenTypes!= '' && this.searchForm.xinwenTypes!= undefined) {
                    params['xinwenTypes'] = this.searchForm.xinwenTypes
                }
                ...
                params['xinwenCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "xinwenCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=xinwen_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xinwenTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            xinwenCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "xinwenCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入集团新闻收藏数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            xinwenCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "xinwenCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            xinwenCollectionTypes:xinwenCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
